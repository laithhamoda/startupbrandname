...
64. `M0` DNS control for `startupbrandname.com`: the domain is still held in the owner's Atom account (parked on Sedo). The owner must get it transferred, or its DNS changed, before it can point to Vercel.
